<template>
  ...
  <div class="nav-container page-component">
      ...
      <div class="order-detail">
        ...
        >
          ...
          <div class="button-wrapper ml20" v-if="orderInfo.orderStatus == 0">
            <div class="v-button" @click="pay1()">支付宝支付</div>
          </div>
           <div class="button-wrapper ml20" v-if="orderInfo.orderStatus == 0">
            <div class="v-button" @click="pay2()">微信支付</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <el-dialog
      :visible.sync="dialogPayVisible2"
      style="text-align: left"
      :append-to-body="true"
      width="500px"
      @close="closeDialog"
...
import alipayApi from "@/api/order/alipay";
export default {
  data() {
    return {
      orderId: null,
  ...
  methods: {
    ...
    pay() {
      if(this.type == 2){
        ...
        this.dialogPayVisible2 = true;
        alipayApi.createNative(this.orderId).then((response) => {
          this.payObj = response.data;
          if (this.payObj.codeUrl == "") {
            this.dialogPayVisible2 = false;
            this.$message.error("支付错误");
          } else {
            this.timer = setInterval(() => {
              this.queryPayStatus(this.orderId);
    ...
    queryPayStatus(orderId) {
      if(this.type == 2){
        weixinApi.queryPayStatus(orderId).then((response) => {
          ...
          alipayApi.queryPayStatus(orderId).then((response) => {
          if (response.message == "支付中") {
            return;
          }
          clearInterval(this.timer);
