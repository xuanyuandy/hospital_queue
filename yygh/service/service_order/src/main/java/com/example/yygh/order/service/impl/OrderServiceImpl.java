...
import com.example.yygh.userclient.PatientFeignClient;
...
//        }
        ...
        if(result.getInteger("code")!=200) {
            ...
            if(orderInfo.getOrderStatus().intValue() == OrderStatusEnum.PAID.getStatus().intValue()) {
                // 此处就是需要取消当前订单并且进行退款处理，这里需要加上支付宝支付和微信支付的判断
                // paymentType judge
                // 1 支付宝 2 微信
                Boolean isRefund = false;
                int type =  paymentService.getType(orderId);
                if(type == 2)
                    isRefund = weixinService.refund(orderId);
                else
                    isRefund = alipayService.refund(orderId);
                if(!isRefund) {
