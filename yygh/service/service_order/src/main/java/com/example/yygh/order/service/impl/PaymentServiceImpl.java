...
import com.example.yygh.hospclient.HospitalFeignClient;
...
public class PaymentServiceImpl extends
    ...
    //更新订单状态
    @Override
    public void paySuccess(String out_trade_no, Map<String, String> resultMap,int type) {
        //1 根据订单编号得到支付记录
        QueryWrapper<PaymentInfo> wrapper = new QueryWrapper<>();
        wrapper.eq("out_trade_no",out_trade_no);
        if(type == 0) wrapper.eq("payment_type", PaymentTypeEnum.WEIXIN.getStatus());
        else wrapper.eq("payment_type", PaymentTypeEnum.ALIPAY.getStatus());
    ...
    @Override
    public int getType(Long orderId) {
        QueryWrapper<PaymentInfo> wrapper = new QueryWrapper<>();
        wrapper.eq("order_id",orderId);
